...
int files_open = 0;					// Controls the number of active files.



struct edfhdrblock *hdrlist[EDFLIB_MAXFILES];		// Pointer to active files, including handy header infos.
...
int edfopen_file_readonly(const char *path, struct edf_hdr_struct *edfhdr, int read_annotations, const char *md5checksum)
{
	...
	memset(edfhdr, 0, sizeof(struct edf_hdr_struct)); 	// Set all memory to the value '0'.
...
int edfclose_file(int handle)
{
	...
	else		// edf-file was opened to read only ..
	{
		...
		{
			annot = annotationslist[handle];	// Point annot to the first annotation

			while(annot->next_annotation)		// If there is a next annotation ..
			{
				annot = annot->next_annotation; // .. point to it ..

				free(annot->former_annotation);	// .. and free the previous one.
			}

			free(annot);				// free the last one.
		}
...
int edflib_version(void) { return EDFLIB_VERSION; }
