...
class edf_param_struct(ct.Structure):                    # this structure contains all the relevant EDF-signal parameters of one signal
    ...
    _fields_ = [("label", ct.c_char*17),        # label (name) of the signal, null-terminated string
        ...
        ("physdimension", ct.c_char*9),        # physical dimension (uV, bpm, mA, etc.), null-terminated string
        ("prefilter", ct.c_char*81),        # null-terminated string
        ("transducer", ct.c_char*81)]        # null-terminated string
